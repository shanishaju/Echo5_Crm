...
exports.AttendanceController = async (req, res) => {
  try {
    ...
    const existing = await attendanceModel.findOne({
      ...
      punchOut: null, //    Changed from { $exists: false }
    });
    ...
    const isNearCompany = isWithinAllowedDistance(
      location.latitude,
      location.longitude,
    ...
    if (!isNearCompany) {
      return res.status(403).json({
        message: "You are not at the allowed location. Please be near the company premises.",
      });
    }
